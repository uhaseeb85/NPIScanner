...
public class SensitiveDataDetector {
    ...
    public List<Detection> detectSensitiveData(LogStatement logStatement, ScanConfiguration config) {
        ...
        if (isExcluded(statement, config)) {
            return detections;
        }
        ...
        // Extract identifiers from StringBuilder/StringBuffer append operations
        // (single-line)
        ...
        for (String objectRef : objectReferences) {
            if (isSensitiveObjectType(objectRef, config.getSensitiveObjectTypes())
                    && !detectedIdentifiers.contains(objectRef)) {
    ...
    /**
     * Extracts variable names from the statement, excluding those within string
     * literals.
     *
     * @param statement the statement to analyze
    ...
    /**
     * Extracts method call names from the statement, excluding those within string
     * literals.
    ...
    /**
     * Extracts identifiers from StringBuilder/StringBuffer append operations
     * (single-line).
     ...
     * @return true if the identifier matches a sensitive object type, false
     *         otherwise
     */
     ...
     * This performs case-insensitive matching for plain keywords and regex matching
     * for patterns.
     *
    ...
    /**
     * Checks if the statement matches any exclusion pattern.
     *
     * @param statement the statement to check
     * @param config    the scan configuration containing exclusion patterns
     * @return true if the statement is excluded, false otherwise
     */
    boolean isExcluded(String statement, ScanConfiguration config) {
        if (statement == null || config == null) {
            return false;
        }

        for (KeywordPattern exclusion : config.getExclusions()) {
            if (exclusion.isRegex()) {
                try {
                    Pattern pattern = Pattern.compile(exclusion.getKeyword(), Pattern.CASE_INSENSITIVE);
                    if (pattern.matcher(statement).find()) {
                        return true;
                    }
                } catch (Exception e) {
                    // Fallback to plain text if regex is invalid
                    if (statement.toLowerCase().contains(exclusion.getKeyword().toLowerCase())) {
                        return true;
                    }
                }
            } else {
    ...
    /**
     * Checks if a log statement contains non-literal content (variables, method
     * calls, etc.).
     * This helps determine if the statement is logging dynamic data or just a
     * static string.
     * 
     * @param statement the log statement to check
     * @return true if the statement contains variables/methods outside of string
     *         literals
     */
